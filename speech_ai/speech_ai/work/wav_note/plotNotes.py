...
class NotePlotter(object):
    ...
    def plot_multiple_notes(self, directory):
        ...
        with open(self.output_file, 'w') as f:
            f.write(lilypond_text)
        command = os.getcwd() + "\\speech_ai\\work\\wav_note\\LilyPond\\usr\\bin\\lilypond.exe "
        command += self.output_file
        print(command)
        os.system(command)
        name = self.output_file.split('\\')[-1].split('.')[0]
        # command = "move " + name + '.pdf examples\\' + name + '.pdf'
        command = "move " + os.getcwd() + '\\' + name + '.pdf ' + os.getcwd() + '\\speech_ai\\work\\note_pdf\\' + name + '.pdf'
        os.system(command)
        print(command)
        # command = "move " + name + '.mid examples\\' + name + '.mid'
        command = "move " + os.getcwd() + '\\' + name + '.mid ' + os.getcwd() + '\\speech_ai\\work\\note_pdf\\' + name + '.mid'
        os.system(command)
        print(command)
