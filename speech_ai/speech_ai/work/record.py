...
######################################时长&音频分割##################################
    ...
    def segment(self,audio_file1):
        ...
        command = ["ffmpeg", "-i", audio_file, "-f", "segment", "-segment_time", str(segment_time), "-y", "{}_%03d{}".format(file_name, file_ext)]
...
################################存储#######################
    def to_mysql(self):
        ...
        final_result = self.Affix_and_evaluation()
        ...
        for i in range(len(xml_list)):
            tmp_xml = self.get_xml_score(xml_list[i])
...
# 连接数据库
        ...
        cur.execute("SELECT MAX(id) FROM speach_table")
        result = cur.fetchone()
        if result[0] is None:
            next_id = 1
        else:
