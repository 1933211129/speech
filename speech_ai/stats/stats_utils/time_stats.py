...
def traffic_recent_hours(n: int) -> list:
    ...
    while (n != 1):
        ...
        # r = round(bytes_recv(t=t, t0=t + 3600, ip="*", path="*") / 1000, 2)
        # s = round(bytes_send(t=t, t0=t + 3600, ip="*", path="*") / 1000, 2)
        recv.append(r)
        send.append(s)
        n = n - 1
    ...
    # recv.append(round(bytes_recv(t=strat_of_this_hour()) / 1000,2))
    # send.append(round(bytes_send(t=strat_of_this_hour()) / 1000,2))
    return [recv, send]
